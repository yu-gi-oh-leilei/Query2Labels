...
def get_datasets(args):
    ...
    if args.dataname == 'coco14' or args.dataname == 'COCO2014':
        ...
        train_dataset = VGDataset(train_dir, train_list, train_data_transform, train_label, args.num_class)
        val_dataset = VGDataset(test_dir, test_list, test_data_transform, test_label, args.num_class)

    elif args.dataname == 'vg1000':
        dataset_dir = args.dataset_dir
        vg_root = osp.join(dataset_dir, 'VG')
        train_dir = osp.join(vg_root,'VG_100K')
        train_list = osp.join(vg_root,'train_list_1000.txt')
        test_dir = osp.join(vg_root,'VG_100K')
        test_list = osp.join(vg_root,'test_list_1000.txt')
        train_label = osp.join(vg_root,'vg_category_1000_labels_index.json')
        test_label = osp.join(vg_root,'vg_category_1000_labels_index.json')

...
import models2
from models2.cls_cvt import build_CvT
from models2.vision_transformer.swin_transformer import build_swin_transformer
from models2.vision_transformer.vision_transformer import build_vision_transformer
...
def get_model_path(modelname):
    ...
    pretrained_dir = '/media/mlldiskSSD/pretrained'
    PTDICT = {
...
class Backbone(BackboneBase):
    ...
    def __init__(self, name: str,
        ...
        if name in ['resnet18', 'resnet50', 'resnet34', 'resnet101']:
            ...
            backbone = getattr(models2, name)(
                {'num_classes': 1}
            )
            # load pretrained model
            if pretrained:
                pretrainedpath = get_model_path(name)
                # pretrainedpath = '/media/data/maleilei/MLIC/query2labels/output/TResNetL_448_COCO/baselinework10/model_best.pth.tar'
                ...
                checkpoint = OrderedDict({k:v for k, v in clean_body_state_dict(checkpoint).items() if 'head.fc' not in k})
...
if __name__ == '__main__':
    ...
    train_backbone = True
    pretrained = True
    backbone = Backbone('tresnetl_v2', train_backbone, return_interm_layers, False, pretrained)
    bb_num_channels = backbone.num_channels
